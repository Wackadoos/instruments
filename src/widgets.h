...
static inline auto SPEED_WIDGET = Widget(&SensorState::temp_motor_back, 240, 70, TextAlign::CENTER, RGB565_ORANGE, 14);
static inline auto POWER_WIDGET = Widget(&SensorState::battery_power, 240, 180, TextAlign::CENTER, RGB565_GOLDENROD, 9);

//* Bottom Row
const inline char MOTOR_TEMP_TEXT[] = "Motor";
const inline char* MOTOR_TEMP_TEXT_PTR = MOTOR_TEMP_TEXT;
static inline auto MOTOR_TEMP_TEXT_WIDGET = StaticWidget(&MOTOR_TEMP_TEXT_PTR, 0, 275, TextAlign::LEFT, RGB565_WHITESMOKE, 2);
static inline auto MOTOR_TEMP1_WIDGET = Widget(&SensorState::temp_motor_front, 0, 295, TextAlign::LEFT, RGB565_LIGHTGREEN, 3);
static inline auto MOTOR_TEMP2_WIDGET = Widget(&SensorState::temp_motor_back, 60, 295, TextAlign::LEFT, RGB565_LIGHTGREEN, 3);
...
static inline auto AMBIENT_TEMP_TEXT_WIDGET = StaticWidget(&AMBIENT_TEMP_TEXT_PTR, 180, 275, TextAlign::LEFT, RGB565_WHITESMOKE, 2);
static inline auto TEMP1_WIDGET = Widget(&SensorState::ambient_temperature, 180, 295, TextAlign::LEFT, RGB565_LIGHTGREEN, 3);
static inline auto TEMP2_WIDGET = Widget(&SensorState::ambient_temperature_2, 240, 295, TextAlign::LEFT, RGB565_LIGHTGREEN, 3);
...
static inline auto SETTINGS_BUTTON = Button(&SETTINGS_TEXT_PTR, 410, 285, 140, 50, 12, RGB565_BLUE, 2);
const inline char RACE_TEXT[] = "";
const inline char* RACE_TEXT_PTR = RACE_TEXT;
static inline auto RACE_BUTTON = Button(&RACE_TEXT_PTR, 425, 215, 100, 80, 12, RGB565_BLUEVIOLET, 2);
...
const static inline WidgetBase* RACE_PAGE_WIDGETS[] = {&BATT_WIDGET, &SPEED_WIDGET, &POWER_WIDGET, &MOTOR_TEMP_TEXT_WIDGET, &ESC_TEMP_TEXT_WIDGET, &AMBIENT_TEMP_TEXT_WIDGET, &MOTOR_TEMP1_WIDGET, &MOTOR_TEMP2_WIDGET, &ESC_TEMP_WIDGET, &TEMP1_WIDGET, &TEMP2_WIDGET, &TIME_WIDGET, &LAP_TIME_WIDGET, &GPS_WIDGET, &SETTINGS_BUTTON, &RACE_BUTTON, &RACE_BUTTON2, &RACE_BUTTON3};
