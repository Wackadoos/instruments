...
- Serial: `Serial2` → VESC/ESC (250000 baud per `README.md`), `Serial3` → GPS. SPI (50/51/52) is shared by display, touchscreen, and SD card via separate CS pins (53/49/48). SD data logging (`src/modules/sd.*`) is implemented: exFAT (SdExFat), a `<date-time>.bin` opened in `SD::startLog()` when RACE mode starts and closed in `SD::stopLog()` (hooked in `State::enterMode`), fixed 138-byte frames (u16 magic 0x4C47 + u32 epoch + u32 millis + 128-byte `LogPayload` of all numeric `State` members), 3h `preAllocate`, a 512-byte `RingBuf` drained in `SD::run()` only when the card isn't busy. The Python decoder lives in `main.py` (`uv run main.py`), matching frame layout in `src/modules/sd.h` — keep them in sync. `Errors::flushToLogfile` is still a TODO.
